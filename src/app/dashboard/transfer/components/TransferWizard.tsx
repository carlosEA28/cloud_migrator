...
import { Search, ChevronRight, ArrowLeft, FolderOpen, CheckCircle2, Loader2 } from "lucide-react"
...
const PROVIDERS: ProviderOption[] = [
  ...
  {
    key: "aws-s3",
    dbKey: "google", // Mock dbKey for now to avoid types error if not handled
    name: "AWS S3",
    logo: googleDriveLogo,
    description: "Cloud storage",
  },
  {
    key: "azure-blob",
    dbKey: "google", // Mock dbKey for now
    name: "Azure Blob",
    logo: googleDriveLogo,
    description: "Cloud storage",
  },
  ...
  useEffect(() => {
    async function loadStatus() {
      try {
        ...
        setConnectedProviders(status.filter((p) => p.connected).map((p) => p.provider))
      } catch (error) {
    ...
    if (step === "origin" && origin) {
      ...
      if (selectedProvider && (selectedProvider.dbKey === "google" || selectedProvider.dbKey === "dropbox")) {
        ...
        try {
          ...
          alert(`Failed to fetch items: ${error instanceof Error ? error.message : "Unknown error"}`)
        } finally {
    ...
    <div className="mx-auto max-w-4xl space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
               ...
               <Button 
                ...
                onClick={() => {
                  if (selectedFiles.length === items.length) setSelectedFiles([])
                  else setSelectedFiles(items.map(f => f.id))
                }}
               >
                 {selectedFiles.length === items.length && items.length > 0 ? "Deselect all" : "Select all"}
               </Button>
            </div>
          </div>
            ...
            {items.length > 0 ? (
              items.map((item) => (
                <Card
                  key={item.id}
                  onClick={() => {
                     setSelectedFiles(prev => prev.includes(item.id) ? prev.filter(f => f !== item.id) : [...prev, item.id])
                  }}
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-right">
                      <p className="text-sm text-white">{item.size ? `${(item.size / 1024 / 1024).toFixed(2)} MB` : "-"}</p>
                    ...
                    <div className={`size-5 rounded border-2 flex items-center justify-center transition-all ${
                      ...
                      {selectedFiles.includes(item.id) && <CheckCircle2 className="size-3 text-white" />}
                    </div>
                  </div>
                </Card>
              ))
      ...
      {step === "summary" && (
        <div className="space-y-6">
            ...
            <Card className="border-[#1F1F2E] bg-[#111118] p-6">
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            <Image src={PROVIDERS.find(p => p.key === origin)?.logo} alt="" width={20} height={20} />
                            <span className="font-medium text-white">{PROVIDERS.find(p => p.key === origin)?.name}</span>
                        </div>
                    </div>
                    <div className="flex items-center justify-between">
                        <span className="text-sm text-[#64748B]">Destination</span>
                        <div className="flex items-center gap-2">
                            <Image src={PROVIDERS.find(p => p.key === destination)?.logo} alt="" width={20} height={20} />
                            <span className="font-medium text-white">{PROVIDERS.find(p => p.key === destination)?.name}</span>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                        <span className="text-sm text-[#64748B]">Estimated Size</span>
                        <span className="font-medium text-[#00FFC8]">~450 MB</span>
                    </div>
                </div>
            </Card>
          ...
          <div className="flex justify-end pt-4">
            <Button
              onClick={() => alert("Transfer started! (Mocked)")}
